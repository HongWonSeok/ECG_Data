...
class ECGClassifierTrainer:
    ...
    def train_epoch(self, epoch):
        ...
        print(type(self.data_loader[Mode.train]))
        ...
        for signal, spectrogram, label in loader:
            self.optimizer.zero_grad()
            
            signal = signal.to(self.device)
            label  = label.to(self.device)
            
            if self.data_loader_args['args']['spectrogram']:
        ...
        train_acc = accuracy.item() / len(loader) / max(1, self.data_args['args']['batch_size'] // 50)
    ...
    def validate_epoch(self, epoch):
        ...
        valid_acc = accuracy.item() / len(loader) / max(1, self.data_args['args']['batch_size'] // 50) # self.data_loader_args['args']['batch_size']
